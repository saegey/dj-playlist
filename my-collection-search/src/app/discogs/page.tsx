...
export default function DiscogsSyncPage() {
  ...
  const [backfillResult, setBackfillResult] = useState<null | { updated: number; failed: string[]; error?: string }>(null);
  ...
  const handleBackfillEmbeddings = async () => {
    ...
    try {
      const res = await fetch("/api/tracks/backfill-embeddings", { method: "POST" });
      ...
      <Box maxW="700px" mx="auto" p={8}>
        ...
        <SimpleGrid gap={4} columns={{ base: 1, md: 3 }}>
          ...
          <Button
            ...
            loading={backfilling}
            disabled={backfilling || syncing || indexing || backingUp}
