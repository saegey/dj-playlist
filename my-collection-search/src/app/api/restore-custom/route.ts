...
/**
 * Restore database from PostgreSQL custom format backup
 * This is more reliable than plain SQL for complex data with special characters
 *
 * POST /api/restore-custom
 * Body: multipart/form-data with 'file' field containing .dump file
 */
export async function POST(request: Request) {
  try {
    ...
    // Save uploaded file to /app/dumps/restore.dump
    const restoreDir = path.resolve("dumps");
    if (!fs.existsSync(restoreDir)) {
      fs.mkdirSync(restoreDir, { recursive: true });
    }
    const restorePath = path.join(restoreDir, "restore.dump");
...
`;
    ...
    // Use pg_restore for custom format files (more reliable than psql)
    // --exclude-table-data: Skip pgmigrations table data to avoid conflicts
    // --single-transaction: Run in a single transaction
    // --no-owner: Don't try to set ownership
    // --no-acl: Don't restore access privileges
    const cmd = `PGPASSWORD='${pass}' pg_restore -U ${user} -h ${host} -p ${port} -d ${db} --exclude-table-data=pgmigrations --single-transaction --no-owner --no-acl '${restorePath}'`;
    ...
    return NextResponse.json({
      message: "Database cleaned and restored successfully (custom format).",
      format: "custom",
      reindex: {
        albumsIndexed,
        tracksIndexed,
