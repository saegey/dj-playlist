import { NextResponse } from "next/server";
import fs from "fs";
import path from "path";
import { execSync } from "child_process";
import { Pool } from "pg";
import { getMeiliClient } from "@/lib/meili";
import { getOrCreateAlbumsIndex, configureAlbumsIndex } from "@/services/albumMeiliService";
import { getOrCreateTracksIndex, configureMeiliIndex } from "@/services/meiliIndexService";
import { addTracksToMeili } from "@/services/meiliDocumentService";

function parsePgUrl(pgUrl: string) {
  try {
    const url = new URL(pgUrl);
    return {
      user: url.username,
      pass: url.password,
      host: url.hostname,
      port: url.port || 5432,
      db: url.pathname.replace(/^\//, ""),
    };
  } catch {
    return {};
  }
}

/**
 * Smart restore that handles both:
 * - Old backups (data-only, need migrations)
 * - New backups (schema + data, don't need migrations)
 *
 * POST /api/restore-smart?dropSchema=true
 * Body: multipart/form-data with 'file' field
 * Query params:
 *   - dropSchema: true to drop and recreate schema (default: false)
 */
export async function POST(request: Request) {
  try {
    // Check if we should drop schema
    const url = new URL(request.url);
    const dropSchema = url.searchParams.get("dropSchema") === "true";

    // Parse multipart form data
    const formData = await request.formData();
    const file = formData.get("file");
    if (!file || typeof file === "string") {
      return NextResponse.json({ error: "No file uploaded" }, { status: 400 });
    }

    // Save uploaded file
    const restoreDir = path.resolve("dumps");
    if (!fs.existsSync(restoreDir)) {
      fs.mkdirSync(restoreDir, { recursive: true });
    }
    const restorePath = path.join(restoreDir, "restore.sql");
    const arrayBuffer = await file.arrayBuffer();
    const fileContent = Buffer.from(arrayBuffer);
    fs.writeFileSync(restorePath, fileContent);

    // Detect if backup includes schema
    const hasSchema = fileContent.toString().includes("CREATE TABLE");
    console.log(`[Restore] Backup type: ${hasSchema ? "schema + data" : "data-only"}`);

    // Get DB connection info
    let pg = {
      url: process.env.DATABASE_URL,
      db: process.env.POSTGRES_DB,
      user: process.env.POSTGRES_USER,
      pass: process.env.POSTGRES_PASSWORD,
      host: process.env.POSTGRES_HOST,
      port: process.env.POSTGRES_PORT,
    };
    if (pg.url) {
      const parsed = parsePgUrl(pg.url);
      pg = {
        ...pg,
        ...{ ...parsed, port: parsed.port ? String(parsed.port) : undefined },
      };
    }
    const db = pg.db || "mydb";
    const user = pg.user || "myuser";
    const pass = pg.pass || "mypassword";
    const host = pg.host || "db";
    const port = pg.port || "5432";

    // Step 1: Clean database
    let cleanScript: string;
    if (dropSchema) {
      console.log("[Restore] Dropping and recreating schema...");
      cleanScript = `
DROP SCHEMA IF EXISTS public CASCADE;
CREATE SCHEMA public;
GRANT ALL ON SCHEMA public TO ${user};
GRANT ALL ON SCHEMA public TO public;
`;
    } else {
      console.log("[Restore] Truncating all tables (keeping schema)...");
      cleanScript = `
DO $$
DECLARE
    r RECORD;
BEGIN
    SET session_replication_role = replica;
    FOR r IN (
        SELECT tablename
        FROM pg_tables
        WHERE schemaname = 'public'
        AND tablename != 'pgmigrations'
    ) LOOP
        EXECUTE 'TRUNCATE TABLE ' || quote_ident(r.tablename) || ' CASCADE';
    END LOOP;
    SET session_replication_role = DEFAULT;
END $$;
`;
    }

    const cleanPath = path.join(restoreDir, "clean.sql");
    fs.writeFileSync(cleanPath, cleanScript);

    const cleanCmd = `PGPASSWORD='${pass}' psql -U ${user} -h ${host} -p ${port} -d ${db} -f '${cleanPath}'`;
    execSync(cleanCmd, {
      stdio: "pipe",
      env: { ...process.env, PGPASSWORD: pass },
    });
    console.log(`[Restore] Database cleaned (${dropSchema ? "schema dropped" : "data truncated"})`);

    // Step 2a: If schema was dropped and backup is data-only, run migrations
    if (dropSchema && !hasSchema) {
      console.log("[Restore] Data-only backup + schema dropped, running migrations...");
      try {
        execSync("npm run migrate up", {
          stdio: "pipe",
          cwd: process.cwd(),
        });
        console.log("[Restore] Migrations completed");
      } catch (migrateError) {
        const errorOutput = migrateError instanceof Error && 'stderr' in migrateError
          ? (migrateError as any).stderr?.toString() || migrateError.message
          : String(migrateError);
        throw new Error(`Migration failed: ${errorOutput}`);
      }
    } else if (!dropSchema && !hasSchema) {
      console.log("[Restore] Data-only backup detected, assuming schema already exists");
    }

    // Step 2b: Restore backup
    console.log("[Restore] Restoring backup...");
    const restoreCmd = `PGPASSWORD='${pass}' psql -U ${user} -h ${host} -p ${port} -d ${db} --single-transaction -v ON_ERROR_STOP=1 -q -f '${restorePath}'`;
    try {
      execSync(restoreCmd, {
        stdio: "pipe",
        env: { ...process.env, PGPASSWORD: pass },
      });
      console.log("[Restore] Backup restored");
    } catch (execError) {
      const errorOutput = execError instanceof Error && 'stderr' in execError
        ? (execError as any).stderr?.toString() || execError.message
        : String(execError);
      throw new Error(`Restore failed: ${errorOutput}`);
    }

    // Step 3: Reindex MeiliSearch
    console.log("[Restore] Reindexing search...");
    let albumsIndexed = 0;
    let tracksIndexed = 0;
    let reindexWarning: string | null = null;

    try {
      const pool = new Pool({ connectionString: process.env.DATABASE_URL });
      const meiliClient = getMeiliClient();

      // Reindex albums
      const albumsIndex = await getOrCreateAlbumsIndex(meiliClient);
      await configureAlbumsIndex(albumsIndex);
      const albumsRes = await pool.query(`
        SELECT
          a.release_id,
          a.friend_id,
          f.username,
          a.title,
          a.artist,
          a.year,
          a.genres,
          a.styles,
          a.album_thumbnail,
          a.discogs_url,
          a.date_added,
          a.date_changed,
          a.track_count,
          a.album_rating,
          a.album_notes,
          a.purchase_price,
          a.condition,
          a.label,
          a.catalog_number,
          a.country,
          a.format,
          a.library_identifier
        FROM albums a
        LEFT JOIN friends f ON f.id = a.friend_id
      `);
      const albumDocs = albumsRes.rows.map((row) => ({
        id: `${row.release_id}_${row.friend_id}`,
        ...row,
      }));
      await albumsIndex.deleteAllDocuments();
      if (albumDocs.length > 0) {
        await albumsIndex.addDocuments(albumDocs);
      }
      albumsIndexed = albumDocs.length;

      // Reindex tracks
      const tracksIndex = await getOrCreateTracksIndex(meiliClient);
      await configureMeiliIndex(tracksIndex, meiliClient);
      const tracksRes = await pool.query(`
        SELECT
          t.*,
          a.library_identifier,
          COALESCE(f.username, t.username) AS username_resolved
        FROM tracks t
        LEFT JOIN albums a
          ON t.release_id = a.release_id AND t.friend_id = a.friend_id
        LEFT JOIN friends f
          ON t.friend_id = f.id
      `);
      const trackDocs = tracksRes.rows.map((row) => {
        const normalized = { ...row };
        normalized.username = row.username_resolved;
        delete normalized.username_resolved;
        return normalized;
      });
      await tracksIndex.deleteAllDocuments();
      if (trackDocs.length > 0) {
        await addTracksToMeili(tracksIndex, trackDocs);
      }
      tracksIndexed = trackDocs.length;

      await pool.end();
      console.log("[Restore] Reindexing complete");
    } catch (reindexError) {
      reindexWarning =
        reindexError instanceof Error
          ? reindexError.message
          : String(reindexError);
    }

    return NextResponse.json({
      message: "Database restored successfully",
      backupType: hasSchema ? "schema + data" : "data-only",
      reindex: {
        albumsIndexed,
        tracksIndexed,
        warning: reindexWarning,
      },
    });
  } catch (e) {
    return NextResponse.json(
      { error: e instanceof Error ? e.message : String(e) },
      { status: 500 }
    );
  }
}
