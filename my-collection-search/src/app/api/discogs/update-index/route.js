...
export async function POST() {
  try {
    if (!fs.existsSync(DISCOGS_EXPORTS_DIR)) {
      return NextResponse.json({ error: 'discogs_exports directory not found' }, { status: 404 });
    }
    ...
    console.log(`[Discogs Index] Found ${files.length} release JSON files in discogs_exports.`);
    if (!files.length) {
      return NextResponse.json({ error: 'No release JSON files found in discogs_exports' }, { status: 404 });
    }
    let allTracks = [];
    for (const file of files) {
      console.log(`[Discogs Index] Processing album file: ${file}`);
      const album = JSON.parse(fs.readFileSync(path.join(DISCOGS_EXPORTS_DIR, file), 'utf-8'));
      const artist_name = album['artists_sort'] || (album.artists && album.artists[0] && album.artists[0].name) || 'Unknown Artist';
      ...
      (album['tracklist'] || []).forEach(track => {
        ...
        allTracks.push({
          track_id,
          title: track['title'],
          artist: track['artists'] ? track['artists'].map(a => a.name).join(', ') : artist_name,
          album: album_title,
          year: album_year,
          styles: album_styles,
          genres: album_genres,
        ...
        console.log(`[Discogs Index] Prepared track: ${track_id} - ${track['title']}`);
      });
    }
    ...
    for (const [i, track] of allTracks.entries()) {
      if (i % 100 === 0) console.log(`[Discogs Index] Upserting track ${i + 1}/${allTracks.length}`);
      ...
      if (rows.length) {
        ...
        for (const field of ['bpm', 'key', 'notes', 'local_tags', 'apple_music_url']) {
    ...
    await index.updateFilterableAttributes(['title',
      ...
      'styles',
      'year']);
    console.log(`[Discogs Index] Adding ${upserted.length} tracks to MeiliSearch index...`);
    ...
    for (const t of upserted) {
      ...
      if (t.apple_music_url === null || t.apple_music_url === undefined) delete t.apple_music_url;
      if (t.duration_seconds === null || t.duration_seconds === undefined) delete t.duration_seconds;
    }
    ...
    console.log(`🚀 Added ${upserted.length} tracks to MeiliSearch (task UID: ${taskUid})`);


    // await index.waitForTask(task.taskUid);
    console.log(`[Discogs Index] Done. Upserted and indexed ${upserted.length} tracks.`);
    await pool.end();
   
    return NextResponse.json({ message: `Upserted and indexed ${upserted.length} tracks from discogs_exports.` });
  } catch (e) {
