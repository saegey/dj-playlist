...
export const PlaylistViewerDrawer = () => {
  ...
  const { playlist, playlistsLoading, exportPlaylist, addToPlaylist, clearPlaylist, sortGreedy, sortGenetic, playlistInfo } = usePlaylists();
  ...
  const saveDialog = usePlaylistSaveDialog(playlistInfo?.id);

  const handleExportPlaylistToPDF = React.useCallback(() => {
    exportPlaylistToPDF({
      playlist: playlist as unknown as Track[],
      totalPlaytimeFormatted,
      filename: `playlist-${new Date().toISOString().split("T")[0]}.pdf`,
    });
  }, [playlist, totalPlaytimeFormatted]);
